...
App.prototype.start = function(cb) {
  ...
  function setup(done) {
    ...
    // Create Child DB
    var childDB = self.children[0] = cp.fork(__dirname + '/db.js',['./test/testClient1']);

    childDB.on('message',function(m) {
      if (m !== 'ready') console.error('Recieved Uknown Command' + m)
      else {

        var manifest = require('../test/testClient1/manifest');
        var db = multilevel.client(manifest);

        // now pipe the db to the server
        var stream = require('shoe')();
