...
App.prototype.start = function(cb) {
  ...
  /**
   * Create DB and execute 'done' callback when recieved 'ready' message from child
   * @param  {Array}         args [self, type, port ]
   * @param  {Function}       done [callback]
   * @return {[Child Fork]}        [child process]
   */
  function createDB(done,opt) {
    var child = self.children[opt[1]] = cp.fork(__dirname + '/db.js',opt);
  ...
  function setup(done) {
    ...
    // Run db.js as child process and add to children list
    async.series({
      state: function(done) {
        createDB(done, [self.settings.path,'state','3000']);
      },
      block: function(done) {
        createDB(done, [self.settings.path,'block','3001']);
      },
      details: function(done) {
        createDB(done, [self.settings.path,'details','3002']);
      }
    }, function(err, result) {
      if(!err){
        // Start Listening
        var stateDB = connectDB(3000),
            blockDB = connectDB(3001),
            detailsDB = connectDB(3002);
