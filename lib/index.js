...
App.prototype.start = function(cb) {
  ...
  function setup(done) {
    ...
    // Set Path DB if Set
    var dbpath = self.settings.path ? self.settings.path : './test/testClient';

    // Create Child DB
    var childDB = self.children[0] = cp.fork(__dirname + '/db.js', [dbpath]);
    ...
    async.series([
        ...
        function(done) {
          jf.readFile(path.join(dbpath,'/manifest'), function(err, manifest) {
            if (err) done(err);
        ...
        function(done, db) {
          // Set Servers
          self.blockDB = db.sublevel('block');
          self.stateDB = db.sublevel('state');
          self.detailsDB = db.sublevel('details');

          //create the blockchain
          self.blockchain = new Ethereum.Blockchain(self.blockDB, self.detailsDB);
          //create a VM
          self.vm = new Ethereum.VM(self.stateDB);
          //start the blockchain. This will lookup last block on the blockchain.
          self.blockchain.init(done);
          // Define DB's from Ports
          self.accountMan = new AccountMan(self.detailsDB);
          done();
        }
      ], function(err) {
    ...
    // // Set Servers
    // self.blockDB = levelup(path + '/block');
    // self.stateDB = levelup(path + '/state');
    // self.detailsDB =levelup(path + '/details');
