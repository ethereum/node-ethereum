...
App.prototype.start = function(cb) {
  ...
  function setup(done) {
    ...
    var childDB = self.children.db = cp.spawn('node', ['db.js']);

    // Send Settings Object to Child Process
    childDB.stdin.write(self.settings);
    ...
    var stateDB = connectDB(3000),
        blockDB = connectDB(4000),
        detailsDB = connectDB(5000);
